...
if ENABLE_SHARK_LARGE then
	...
	mobs:register_mob("mobs_sharks:shark_lg", {
		...
		jump = false,
		stepheight = 0,
		drops = {{name = "mobs:meat_raw", chance = 1, min = 1, max = 3},},

   --impedir o peixe de encalhar fora da agua
    --busca um local com agua
    on_flop = function(self)
		local posicao = self.object:get_pos()
		posicao.x = posicao.x+math.random(-6, 6)
		posicao.y = posicao.y+math.random(-3, 0)
		posicao.z = posicao.z+math.random(-6, 6)
		local node = minetest.get_node(posicao)
		if node.name == "default:water_source"then
            self.object:set_pos(posicao)
		end
	end
	...
	mobs:register_egg("mobs_sharks:shark_lg", "Shark (large)", "mob_shark_shark_item.png", 0)
end
